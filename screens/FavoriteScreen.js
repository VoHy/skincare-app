...
export default function FavoriteScreen() {
    ...
    const loadFavorites = useCallback(async () => {
        ...
        try {
            ...
            if (storedFavorites) {
                ...
                const favoritesArray = Array.isArray(parsedFavorites) ? parsedFavorites : Object.values(parsedFavorites);
    ...
    const toggleFavorite = async (product) => {
        try {
            ...
            if (index !== -1) {
                // If the product exists, remove it from the favorites list
            ...
            setFavorites(favorites); // Update local state
        } catch (error) {
            ...
            [
                { text: "Hủy", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            if (!userId) {
                                Alert.alert("Thông báo", "Bạn cần đăng nhập để xóa danh sách yêu thích.");
                                return;
                            }
    ...
    const renderFavoriteItem = ({ item }) => (
        <View style={styles.favoriteItem}>
            ...
            <View style={styles.details}>
                ...
                <Text style={styles.price}>Giá: {item.discount_price.toLocaleString()} VND</Text>
                <Text style={styles.originalPrice}>Giá gốc: {item.price.toLocaleString()} VND</Text>
                <TouchableOpacity onPress={() => toggleFavorite(item)}>
                    <AntDesign
                        name={favorites.find(fav => fav._id === item._id) ? "heart" : "hearto"}
                        size={24}
                        color="red"
                    />
                </TouchableOpacity>
    ...
    return (
        <View style={styles.container}>
            {favorites.length > 0 ? (
                ...
                <Text style={styles.noFavoritesText}>Bạn cần đăng nhập để xem sản phẩm yêu thích!</Text>
            )}
            {favorites.length >= 2 && (
